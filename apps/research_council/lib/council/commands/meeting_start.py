...
from council.interactive.state import ensure_interactive_fields
...
def cmd_start(args: argparse.Namespace) -> None:
    ...
    publish_meeting_started(meeting_event_log(meeting_dir, meeting_id), state)
    ...
    elif meeting_mode == "interactive" and not scenario:
        print(f"Mode: interactive (turn-based) | Max rounds: {state['max_rounds']}")
        print(f"Rounds before owner pause: {owner_pause}")
        print("Next: ./council.sh context \"范围\" && ./council.sh select ... && ./council.sh run-interactive")
    elif not scenario:
