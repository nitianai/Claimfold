...
# App（应用层）根：apps/research_council/
APP_ROOT = Path(__file__).resolve().parent.parent.parent
# 仓库根：Claimfold/（运行时数据默认在此）
REPO_ROOT = APP_ROOT.parent.parent
_data_root = os.environ.get("COUNCIL_DATA_ROOT", "").strip()
DATA_ROOT = Path(_data_root).resolve() if _data_root else REPO_ROOT

# 向后兼容：ROOT 指向 App 资产目录
ROOT = APP_ROOT

CURRENT_MEETING_FILE = DATA_ROOT / ".current_meeting"
MEETINGS_DIR = DATA_ROOT / "meetings"
CLAIMS_DIR = DATA_ROOT / "claims"
CONFIG_FILE = APP_ROOT / "config" / "guests.yaml"
INIT_CONFIG_TEMPLATE = APP_ROOT / "config" / "guests.yaml.template"
GUEST_TEMPLATE = APP_ROOT / "prompts" / "guest" / "template.md"
GUEST_JSON_TEMPLATE = APP_ROOT / "prompts" / "guest" / "json.md"
GUEST_RESEARCH_TEMPLATE = APP_ROOT / "prompts" / "guest" / "research.md"
INVESTMENT_GUEST_TEMPLATE = APP_ROOT / "prompts" / "guest" / "investment.md"
SUMMARIZER_TEMPLATE = APP_ROOT / "prompts" / "system" / "summarizer.md"
MARKET_CONTEXT_PROMPT = APP_ROOT / "prompts" / "system" / "market_context.md"
INVESTMENT_REPORT_PROMPT = APP_ROOT / "prompts" / "reports" / "investment_report.md"
SCRIPTS_DIR = APP_ROOT / "scripts"
SCENARIOS_DIR = APP_ROOT / "scenarios"
ROLES_FILE = APP_ROOT / "config" / "roles.yaml"
EXECUTORS_FILE = APP_ROOT / "config" / "executors.yaml"
BINDINGS_DIR = APP_ROOT / "config" / "bindings"
