...
def load_guests_for_meeting(meeting_dir: Path | None = None) -> dict[str, Any]:
    from council.guest_overrides import merge_guest_config

    return merge_guest_config(load_guests(), meeting_dir)
...
def is_research_mode(state: dict[str, Any]) -> bool:
    return (
        state.get("output_format") == "research"
        or state.get("round_mode") in ("parallel", "interactive")
        or state.get("meeting_mode") == "interactive"
    )
