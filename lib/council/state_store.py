...
from council.config import CURRENT_MEETING_FILE, LEGACY_GUEST_MAP, MEETINGS_DIR
...
from utils import atomic_write_json, resolve_meeting_path, validate_meeting_id
...
def save_state(meeting_dir: Path, state: dict[str, Any]) -> None:
    atomic_write_json(meeting_dir / "meeting_state.json", state)
...
def get_current_meeting_dir() -> Path:
    ...
    meeting_id = validate_meeting_id(CURRENT_MEETING_FILE.read_text(encoding="utf-8"))
    meeting_dir = resolve_meeting_path(MEETINGS_DIR, meeting_id)
