...
def compute_metrics(meeting_dir: Path, state: dict[str, Any]) -> dict[str, Any]:
        ...
        if h.get("mode") == "parallel" and "entries" in h:
            for e in h["entries"]:
                ...
                if not e.get("success"):
                    failures += 1
                if e.get("duration_s"):
                    durations.append((e.get("guest", "?"), e["duration_s"]))
            ...
            oq_added += h.get("open_questions_added", 0)
            if h.get("duration_s"):
                durations.append((h.get("guest", "?"), h["duration_s"]))
            if h.get("success") is False or h.get("validation_errors"):
                failures += 1
